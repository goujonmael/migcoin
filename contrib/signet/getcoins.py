...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing migcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='migcoin-cli', help='migcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Migcoin address to which the faucet should send')
...
parser.add_argument('migcoin_cli_args', nargs='*', help='Arguments to pass on to migcoin-cli (default: -signet)')

args = parser.parse_args()

if args.migcoin_cli_args == []:
    args.migcoin_cli_args = ['-signet']


def migcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.migcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = migcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = migcoin_cli(['getnewaddress', 'faucet', 'bech32'])
