...
Migcoin Core version 25.0 is now available from:

  <https://migcoincore.org/bin/migcoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/migcoin/migcoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `migcoin-cli` tool both take advantage of this, so
  for example:

```sh
migcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
migcoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Migcoin Core will now return an error at startup. (#25574)
...
before Migcoin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching migcoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/migcoin/migcoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
