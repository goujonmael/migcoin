...
#include <config/migcoin-config.h>
#endif

#include <qt/migcoin.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<MigcoinUnit>("MigcoinUnit");
#else
    qRegisterMetaType<MigcoinUnit>("MigcoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. migcoin_de.qm (shortcut "de" needs to be defined in migcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. migcoin_de_DE.qm (shortcut "de_DE" needs to be defined in migcoin.qrc)
...
static const char* qt_argv = "migcoin-qt";

MigcoinApplication::MigcoinApplication()
...
void MigcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the MigcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", MigcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void MigcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool MigcoinApplication::createOptionsModel(bool resetSettings)
{
...
void MigcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new MigcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &MigcoinGUI::quitRequested, this, &MigcoinApplication::requestShutdown);
...
void MigcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void MigcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool MigcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void MigcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &MigcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &MigcoinApplication::handleRunawayException);
    connect(this, &MigcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &MigcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void MigcoinApplication::parameterSetup()
{
...
void MigcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void MigcoinApplication::requestInitialize()
{
...
void MigcoinApplication::requestShutdown()
{
...
void MigcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // migcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MigcoinGUI::handlePaymentRequest);
            connect(window, &MigcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void MigcoinApplication::handleRunawayException(const QString &message)
{
...
void MigcoinApplication::handleNonFatalException(const QString& message)
{
...
WId MigcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool MigcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MigcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(migcoin);
    Q_INIT_RESOURCE(migcoin_locale);
    ...
    /// 6-7. Parse migcoin.conf, determine network, switch to network specific
    ...
    // migcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
