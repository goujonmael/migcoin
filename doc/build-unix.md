UNIX BUILD NOTES
====================
Some notes on how to build Migcoin Core in Unix.
...
memory available when compiling Migcoin Core. On systems with less, gcc can be
...
To build Migcoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build migcoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a migcoin-qt executable will be
built by default.
...
...
to: /path/to/migcoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/migcoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Migcoin Core can
