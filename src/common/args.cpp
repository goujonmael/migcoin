...
const char * const MIGCOIN_CONF_FILENAME = "migcoin.conf";
const char * const MIGCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //migcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", MIGCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Migcoin
    // macOS: ~/Library/Application Support/Migcoin
    // Unix-like: ~/.migcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Migcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Migcoin";
#else
    // Unix-like
    return pathRet / ".migcoin";
#endif
#endif
}
