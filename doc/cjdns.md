# CJDNS support in Migcoin Core

It is possible to run Migcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Migcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Migcoin network and individual nodes.
...
before it will work with your Migcoin Core node. This is described in steps
...
## Run Migcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Migcoin Core
...
When enabled, this option tells Migcoin Core that it is running in an
...
IPv6 local network. This helps Migcoin Core perform better address management:
...
CJDNS support was added to Migcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `migcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/migcoin/migcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Migcoin Core

There are several ways to see your CJDNS address in Migcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `migcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `migcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `migcoin-cli help getnodeaddresses` for details.
