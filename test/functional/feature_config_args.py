...
class ConfArgsTest(MigcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in migcoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "migcoin_bad.conf"
        ...
        with open(os.path.join(self.nodes[0].datadir, 'migcoin.conf'), 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "migcoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by migcoind.
        env, default_datadir = util.get_temp_default_datadir(pathlib.Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a migcoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = pathlib.Path(node.migcoinconf).read_text()
        conf_path = default_datadir / "migcoin.conf"
        ...
        # (conf_path, not node.migcoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a migcoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "migcoin.conf" file which is ignored, because a '
        ...
        # the same migcoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir}/migcoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when migcoin.conf in the default data directory sets another datadir '
                      'and it contains a different migcoin.conf file that would be ignored')
        ...
        # migcoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "migcoin.conf").write_text(f"datadir={node.datadir}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "migcoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/migcoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "migcoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = os.path.join(default_data_dir, "migcoin.conf")
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_migcoin.conf")
