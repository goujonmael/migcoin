...
/** Call node.updateRwSetting() with Migcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Migcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/migcoin/migcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because migcoin 22.x will not try to read these.
...
//! Convert enabled/size values to migcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from migcoin -prune setting.
...
//! Get pruning size value to show in GUI from migcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayMigcoinUnit")) {
        settings.setValue("DisplayMigcoinUnit", QVariant::fromValue(MigcoinUnit::MIG));
    }
    QVariant unit = settings.value("DisplayMigcoinUnit");
    if (unit.canConvert<MigcoinUnit>()) {
        m_display_migcoin_unit = unit.value<MigcoinUnit>();
    } else {
        m_display_migcoin_unit = MigcoinUnit::MIG;
        settings.setValue("DisplayMigcoinUnit", QVariant::fromValue(m_display_migcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if migcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_migcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<MigcoinUnit>() == m_display_migcoin_unit) return;
    m_display_migcoin_unit = new_unit.value<MigcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayMigcoinUnit", QVariant::fromValue(m_display_migcoin_unit));
    Q_EMIT displayUnitChanged(m_display_migcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/migcoin/migcoin/pull/8273
    ...
    // (https://github.com/migcoin-core/gui/issues/567).
