...
""" Example logging Migcoin Core mempool events using the mempool:added,
...
def main(migcoind_path):
    migcoind_with_usdts = USDT(path=str(migcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    migcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    migcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    migcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    migcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[migcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/migcoind")
        exit(1)
