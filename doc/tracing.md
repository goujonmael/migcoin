# User-space, Statically Defined Tracing (USDT) for Migcoin Core

Migcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Migcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Migcoin Core
...
Multiple tools can list the available tracepoints in a `migcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Migcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/migcoind
stap net        outbound_message 0x0000000000107c05 /src/migcoind
stap validation block_connected  0x00000000002fb10c /src/migcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Migcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/migcoind | grep NT_STAPSDT -A 4 -B 2
