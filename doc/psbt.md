# PSBT Howto for Migcoin Core

Since Migcoin Core 0.17, an RPC interface exists for Partially Signed Migcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/migcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Migcoin transactions that are not fully signed
...
#### Multisig with multiple Migcoin Core instances
...
Migcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `migcoin-cli` is used.
...
Later, when *V* MIG has been received on *Amulti*, and Bob and Carol want to
